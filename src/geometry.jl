...
export mesh, SimpleLine, SimpleLineGrid, SimpleRect, SimpleRectGrid
...
function mesh(::Val{:SimpleGrid}, geo::SimpleLine, dξ::Number; ax_ratio=20)
    ξ, nξ, aξ = __divide_segment__(Val(:halfspace), promote(geo.span[1], dξ)...)
    return SimpleLineGrid(ξ, convert(eltype(ξ), dξ), nξ, aξ)
end

function mesh(::Val{:SimpleGrid}, geo::SimpleRect, dx::Number, dξ::Number)
