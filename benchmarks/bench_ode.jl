...
begin
    mesh = gen_mesh(Val(:RectOkada), 10., 10., 10.0/1000, 10.0/1000, 90.)
...
begin
    mf = gen_mesh(Val(:RectOkada), 10., 10., 10.0/1000, 10.0/1000, 90.)
    nx = ny = nz = 20
    nume = prod([nx, ny, nz])
    ...
    gg = ViscoelasticCompositeGreensFunction(ee, ev, ve, vv, nx * ny * nz, 6, 6, ϵcomp, σcomp)

    pe = RateStateQuasiDynamicProperty([rand(mf.nx, mf.nξ) for _ in 1: 4]..., rand(4)...)
    pdisl = DislocationCreepProperty([rand(length(nume)) for _ in 1: 10]...)
    pdiff = DiffusionCreepProperty([rand(length(nume)) for _ in 1: 11]...)
    pc = compose(pe, rand(6), pdisl, pdiff)
    v0 = rand(mf.nx, mf.nξ)
    θ0 = rand(mf.nx, mf.nξ)
    ϵ0 = rand(length(nume), 6)
    σ0 = rand(length(nume), 6)
    δ0 = rand(mf.nx, mf.nξ)
